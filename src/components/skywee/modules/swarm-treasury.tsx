...
const STATUS_BADGE: Record<TreasuryProposal["status"], string> = {
  voting: "bg-foreground/10 text-foreground border-foreground/20",
  executed: "bg-foreground text-background border-foreground",
  rejected: "bg-foreground/[0.03] text-foreground/30 border-foreground/5 line-through",
}
    ...
    <div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-2xl font-bold text-foreground">SwarmTreasury</h3>
          </div>
        ...
        <div className="flex gap-2">
          <div className="px-3 py-1.5 rounded-md skywee-hairline bg-foreground/[0.03]">
            <div className="text-[10px] font-mono uppercase text-foreground/40">Treasury</div>
            <div className="text-sm font-bold text-foreground skywee-tabular">{fmt.usd(8_412_000)}</div>
          </div>
          <div className="px-3 py-1.5 rounded-md skywee-hairline bg-foreground/[0.03]">
            <div className="text-[10px] font-mono uppercase text-foreground/40">Open votes</div>
            <div className="text-sm font-bold text-foreground skywee-tabular">
              {TREASURY_PROPOSALS.filter((p) => p.status === "voting").length}
            </div>
          </div>
          <div className="px-3 py-1.5 rounded-md skywee-hairline bg-foreground/[0.03]">
            <div className="text-[10px] font-mono uppercase text-foreground/40">Agents</div>
            <div className="text-sm font-bold text-foreground skywee-tabular">4</div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-xs font-mono uppercase tracking-wider text-foreground/50">
              ...
              {DELIBERATION_LOG.map((entry, i) => (
                ...
                >
                  <div className="flex-shrink-0 mt-1">
                    <div className="h-7 w-7 rounded-md bg-foreground/5 border border-foreground/10 grid place-items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 text-xs">
                      <span className="font-mono font-semibold text-foreground">{entry.agent}</span>
                      <span className="text-foreground/40">·</span>
                      <span className="text-foreground/60">{entry.role}</span>
                      <span className="ml-auto font-mono text-[10px] text-foreground/30">{entry.t}</span>
                    </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-3 text-[10px] text-foreground/40">
                    <span>by <span className="text-foreground/60 font-mono">{ROLE_LABEL[p.proposedBy]}</span></span>
                    <span>·</span>
                  ...
                  {p.status === "voting" && (
                    <div className="mt-3">
                      <div className="flex items-center justify-between text-[10px] font-mono text-foreground/50 mb-1">
                        <span className="text-foreground/70">FOR {fmt.num(p.votesFor)}</span>
                        <span className="text-foreground/40">{forPct.toFixed(1)}%</span>
                        <span className="text-foreground/40">AGAINST {fmt.num(p.votesAgainst)}</span>
                      </div>
      ...
      <div className="mt-6 flex items-center justify-between rounded-lg skywee-hairline bg-foreground/[0.02] p-4">
        <div>
          ...
          <div className="text-xs text-foreground/50">Submit an action to the agent swarm for deliberation.</div>
        </div>
        <button
          type="button"
